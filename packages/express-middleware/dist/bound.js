import { createHash, createHmac, randomBytes as secureRandomBytes, randomUUID, timingSafeEqual, } from "node:crypto";
import { Buffer } from "buffer";
import { Address, StrKey } from "@stellar/stellar-sdk";
import { BOUND_PAYMENT_CAPABILITY, BOUND_PAYMENT_SCHEME, REAPP_PAYMENT_CAPABILITIES_HEADER, X_PAYMENT_HEADER, boundChallengeAuthorizationBytes, canonicalPaymentOrigin, decodePaymentProof, isBoundPaymentProof, toStroops, verifyBoundPaymentProofSignature, } from "@ackrate/core";
import { TESTNET } from "@ackrate/stellar";
import { createStellarPaymentVerifier } from "./verification.js";
import { REAPP_PAYMENT_LOCALS_KEY, createRedemptionKey, } from "./middleware.js";
export const REAPP_BOUND_DELIVERY_LOCALS_KEY = "reappBoundDelivery";
export function getBoundDeliveryContext(response) {
    return response.locals[REAPP_BOUND_DELIVERY_LOCALS_KEY];
}
...
function oneRawHeader(request, name, maxBytes) {
    const values = [];
    ...
    if (values.length === 0)
        return undefined;
    if (values.length !== 1)
        throw new Error(`multiple ${name} headers are not allowed`);
...
function authenticateChallenge(unsigned, secret) {
    const mac = createHmac("sha256", secret)
        .update(boundChallengeAuthorizationBytes(unsigned))
        .digest("base64");
    return Object.freeze({
        ...unsigned,
        authorization: Object.freeze({ algorithm: "hmac-sha256", mac }),
    });
}
function challengeMacIsValid(challenge, secret) {
...
export function createBoundReappPaymentMiddleware(options) {
    if (!options || typeof options !== "object") {
        throw new Error("REAPP bound payment middleware options are required.");
    }
    if (!options.redemptionStore
    ...
    if (typeof options.audience === "string")
        canonicalPaymentOrigin(options.audience, "audience");
    ...
    const challengeSecret = Buffer.from(typeof options.challengeSecret === "string"
    ...
    function requirementFor(request) {
    ...
    function challengeFor(request, requirement, capturedNow, audience) {
        const entropy = Buffer.from(randomBytes(32));
        if (entropy.length !== 32)
            throw new Error("randomBytes must return exactly the requested bytes");
        const unsigned = {
            proofVersion: 2,
    ...
    const handle = async (request, response, next) => {
        ...
        try {
            ...
            audience = canonicalPaymentOrigin(resolveRequestValue("audience", options.audience, request), "audience");
        ...
        const exactChallenge = challenge.proofVersion === 2
        ...
        const proofDigest = createHash("sha256")
            ...
            .digest("hex");
        const redemptionKey = createRedemptionKey(networkConfig.networkPassphrase, registryId, proof.txHash);
        const paymentMatchesProof = (payment) => payment.txHash === proof.txHash
        ...
        if (existing.kind === "completed") {
            if (existing.record.key !== redemptionKey
        ...
        let claimed;
        try {
            claimed = await options.redemptionStore.claim(record, randomUUID(), capturedNow);
            if (!claimed
        ...
        if (claimed.record.key !== redemptionKey
