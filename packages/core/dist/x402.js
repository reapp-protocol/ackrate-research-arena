...
export function canonicalPaymentOrigin(value, label = "challenge audience") {
...
export function parseBoundPaymentChallenge(value) {
...
export function boundChallengeAuthorizationBytes(challenge) {
...
export function hashBoundPaymentChallenge(challenge) {
...
export function verifyBoundPaymentProofSignature(proof, agent) {
    try {
        if (proof.authorization.algorithm !== "stellar-ed25519-sha256")
            return false;
        const signature = Buffer.from(canonicalBase64(proof.authorization.signature, 64, "proof signature"), "base64");
        return Keypair.fromPublicKey(agent).verify(boundProofDigest({ challenge: proof.challenge, txHash: proof.txHash, mandateId: proof.mandateId }), signature);
    }
    catch {
        return false;
    }
}
export async function parse402(response) {
    let body;
    try {
...
function parseBoundPaymentProof(value) {
    exactKeys(value, [
        "proofVersion", "scheme", "network", "txHash", "mandateId", "challenge", "authorization",
    ], "bound payment proof");
    if (value.proofVersion !== 2)
        throw new Error("x402: unsupported bound proof version");
    ...
    if (!HEX_32.test(txHash))
        throw new Error("x402: proof txHash must be 32-byte hex");
    if (!HEX_32.test(mandateId))
        throw new Error("x402: proof mandateId must be 32-byte hex");
...
export function decodePaymentProof(header) {
    ...
    if ("proofVersion" in proof) {
        if (proof.proofVersion === 2)
            return parseBoundPaymentProof(proof);
        throw new Error("x402: unsupported payment proof version");
    }
    for (const field of ["scheme", "network", "txHash", "mandateId", "amount"]) {
...
export function isBoundPaymentProof(proof) {
