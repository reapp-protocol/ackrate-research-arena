...
import { REAPP_AP2_CREDENTIAL_VERSION, REAPP_AP2_SIGNATURE_ALGORITHM, ap2CredentialSigningDigest, decodeCanonicalSignature, parseSignedAp2Mandate, rebuildCredentialBinding, } from "./credential.js";
 ...
 */
export function createAp2ComplianceValidator(options) {
    ...
    if (typeof options.replayNamespace !== "string" ||
    ...
    return Object.freeze({
        async validateAndConsume(input) {
            ...
            if (credential.credentialVersion !== REAPP_AP2_CREDENTIAL_VERSION ||
                ...
                throw new Ap2ValidationError("UNSUPPORTED_VERSION", "credential, AP2, binding, data-key, or signature version is unsupported.");
            }
            if (!StrKey.isValidEd25519PublicKey(credential.payload.stellar.user) ||
            ...
            if (credential.payload.stellar.user !== input.expectedUser) {
                throw new Ap2ValidationError("SIGNER_MISMATCH", "credential signer does not match the trusted expected user.");
            }
            ...
            if (binding.mandate.id !== credential.mandateHash) {
                throw new Ap2ValidationError("BINDING_MISMATCH", "credential payload does not match its mandate hash.");
            }
            let signature;
            try {
                ...
                throw new Ap2ValidationError("INVALID_SIGNATURE", "credential signature encoding is invalid.", { cause });
            }
            const digest = ap2CredentialSigningDigest(credential.credentialVersion, credential.payload, credential.mandateHash);
            ...
            if (input.merchant !== credential.payload.intent.merchants[0]) {
                throw new Ap2ValidationError("MERCHANT_MISMATCH", "requested merchant is outside the signed mandate scope.");
            }
            let amountStroops;
            try {
            ...
            if (amountStroops > binding.mandate.maxAmount) {
                throw new Ap2ValidationError("AMOUNT_EXCEEDS_MANDATE", "requested amount exceeds the signed mandate maximum.");
            }
            ...
            if (replayResult !== "consumed") {
                throw new Ap2ValidationError("REPLAY_STORE_UNAVAILABLE", "replay store returned an unsupported result.");
            }
            return Object.freeze({
