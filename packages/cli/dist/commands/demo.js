...
import { TESTNET, registryClient, keypairSigner, token } from "@ackrate/stellar";
import { Keypair, rpc } from "@stellar/stellar-sdk";
import { log, c, banner, link, explorer } from "../ui.js";
import { acknowledgeCompletedSettlement, assertNoPendingSettlement, claimPendingSettlement, clearPendingSettlement, markSettlementCompleted, } from "../settlement-store.js";
...
async function attemptPurchase(agent) {
    ...
    try {
        ...
        if (code === "6")
            return { kind: "blocked" }; // BudgetExceeded — the aha
        if (code === "8")
            return { kind: "retry" }; // BadSequence — stale read, wait & retry
...
function listDemos() {
    ...
    for (const d of DEMOS) {
        log.step(d.id, { run: `ackrate demo ${d.id}` });
        console.log("  " + c.gray(d.summary));
    }
    console.log();
}
export async function runDemo(target) {
    if (!target) {
        listDemos();
        return;
    }
    ...
    await ackrate.registerMandate(mandate, { signer: user.secret() });
    await ackrate.approveBudget(mandate, { signer: user.secret() });
    ...
    const paymentAgent = ackrate.agent({ mandate, signer: agent });
    let purchased = 0;
    let seq = 0;
    ...
    const passed = purchased === 3
        && budgetBlocked
        && finalMandate.spent === 30000000n
        && Number(finalMandate.seq) === 3
        && transferred === 30000000n;
    console.log("\n" +
        c.bold("Result") +
        "\n" +
        ...
        "\n");
    if (!passed) {
        throw new Error(`demo evidence mismatch: purchased=${purchased}, blocked=${budgetBlocked}, seq=${Number(finalMandate.seq)}, spent=${finalMandate.spent}, transferred=${transferred}`);
    }
}
