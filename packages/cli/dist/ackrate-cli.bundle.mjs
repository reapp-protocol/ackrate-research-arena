...
// packages/core/dist/payment-sequence.js
...
async function runDemo(target) {
  ...
  const mandate2 = ackrate.createIntentMandate(inputs);
  await ackrate.registerMandate(mandate2, { signer: user.secret() });
  await ackrate.approveBudget(mandate2, { signer: user.secret() });
  ...
  const paymentAgent = ackrate.agent({ mandate: mandate2, signer: agent });
  let purchased = 0;
  let seq = 0;
...
program2.name("ackrate").description("ackrate agent payments on Stellar, enforced on-chain").version(CLI_VERSION);
